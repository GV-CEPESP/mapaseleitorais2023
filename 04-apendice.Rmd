...
Nesta parte, apresentamos o script utilizado na montagem das bases de dados dos candidatos.
...
Votação por seção eleitoral dos candidatos Guilherme Boulos, Eduardo Suplicy, Fernando Holiday e Milton Leite na cidade de São Paulo em 2020. Fonte: TSE.

```{r electionsBR-sp-2020, message = FALSE, eval = F}
# Essa funcao esta somente na versao em dev do electionsBR
...
base2020 <- base2020 |> 
  # Filtra pelo municipio de SP, 1o turno e elimina brancos/nulos
  filter(NM_MUNICIPIO == "SÃO PAULO",
         NR_TURNO == 1,
         !NR_VOTAVEL %in% c(95, 96)) |> 
  # Calcula o total de votos validos por cargo/secao
  group_by(DS_CARGO,
           NR_ZONA, NR_SECAO) |> 
...
Os dados abaixo foram obtidos do Github do Prof. Daniel Hidalgo (MIT). Seu repositório disponibiliza locais de votação entre 2006 e 2020 para todo o Brasil.
...
# Fonte: IBGE (https://www.ibge.gov.br/geociencias/organizacao-do-territorio/malhas-territoriais/26565-malhas-de-setores-censitarios-divisoes-intramunicipais.html?edicao=26589&t=downloads)
...
# Informacoes da sessao utilizada na producao final dos bancos
sessionInfo()
